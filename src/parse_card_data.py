...
def parse_card_text(card_text):
    ...
    # Extract types
    types_match = re.search(patterns.types_pattern, card_text)
    parsed_data['types'] = [card_type.strip() for card_type in types_match.group(1).split(",")] if types_match else []
    ...
    # Extract resistances
    resistances_type_match = re.findall(patterns.resistances_type_pattern, card_text)
    resistances_value_match = re.findall(patterns.resistances_value_pattern, card_text)
    parsed_data['resistances'] = []
    for i in range(min(len(resistances_type_match), len(resistances_value_match))):
        resistance_data = {
            'type': resistances_type_match[i].strip(),
            'value': resistances_value_match[i].strip()
        }
        parsed_data['resistances'].append(resistance_data)
